...
function updateDailyGoal(){
  const user=db.users[currentUser];if(!user)return;const goal=user.dailyGoal||10,today=localDateKey(new Date()),done=historyFor(currentUser).filter(x=>localDateKey(x.date)===today).length,percent=Math.min(100,Math.round(done/goal*100));
  $('dailyGoalBox').innerHTML=`<div class="daily-goal-head"><span>Objetivo diario: ${done} de ${goal}</span><select id="dailyGoalSelect">${[5,10,15,20,30].map(n=>`<option value="${n}" ${n===goal?'selected':''}>${n} ejercicios</option>`).join('')}</select></div><div class="goal-track"><div class="goal-fill" style="width:${percent}%"></div></div><div class="goal-done-text">${done>=goal?'★ ¡Objetivo conseguido hoy!':'Sigue practicando para completar el día'}</div>`;
  $('dailyGoalSelect').onchange=e=>{user.dailyGoal=+e.target.value;saveDB();updateDailyGoal()};
}
...
document.querySelectorAll('.operation-choice').forEach(b=>b.onclick=()=>chooseOperation(b.dataset.op));
...
function renderCalendar(hist){
  ...
  for(let day=1;day<=days;day++){const key=`${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;cells+=`<button class="calendar-day ${active.has(key)?'has-work':''} ${dayCounts[key]>=goal?'goal-done':''} ${selectedHistoryDate===key?'selected':''}" data-history-day="${key}" title="${dayCounts[key]||0} ejercicios">${day}</button>`}
  cells+='</div>';
...
$('backBtn').onclick=()=>{if(sessionMode==='single'){showScreen('difficultyScreen');renderDifficulty()}else{showScreen('programScreen');renderProgram()}};
